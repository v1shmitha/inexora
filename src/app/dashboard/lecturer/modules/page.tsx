...
import {
  Layers, Plus, Loader2, BookOpen, Search, X, ChevronDown,
  Users, ClipboardList, Pencil, Trash2, ArrowRight,
} from "lucide-react";
import { api } from "~/trpc/react";

interface ModuleForm {
  programId: string; title: string; code: string;
  description: string; isMandatory: boolean; orderIndex: string;
}

const BLANK: ModuleForm = {
  programId: "", title: "", code: "", description: "", isMandatory: true, orderIndex: "",
};
...
const fieldColor = (f?: string) => FIELD_COLORS[f ?? ""] ?? "bg-slate-50 text-slate-600 border-slate-100";
...
export default function ModulesPage() {
  ...
  // staleTime: 0 = always fetch fresh on mount/refresh — fixes "Unknown" bug
  ...
  const approvedPrograms = programs.filter((p) => p.approvalStatus === "APPROVED");

  const createModule = api.course.createModule.useMutation({
    onSuccess: () => { void utils.course.getMyModules.invalidate(); closeModal(); },
  ...
  const openCreate = () => { setEditingId(null); setForm(BLANK); setFormError(null); setModalOpen(true); };
  ...
  const closeModal = () => { setModalOpen(false); setEditingId(null); setForm(BLANK); setFormError(null); };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateModule.mutate({ id: editingId, title: form.title.trim(), code: form.code || null, description: form.description || null, isMandatory: form.isMandatory, orderIndex: form.orderIndex ? parseInt(form.orderIndex) : 0 });
    } else {
      if (!form.programId) { setFormError("Please select a program."); return; }
      createModule.mutate({ programId: form.programId, title: form.title.trim(), code: form.code || null, description: form.description || null, isMandatory: form.isMandatory, orderIndex: form.orderIndex ? parseInt(form.orderIndex) : 0 });
    }
  };
  ...
  const grouped = filtered.reduce<Record<string, typeof filtered>>((acc, mod) => {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Course units you teach within institution programs</p>
          </div>
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-3">
          {[
            { label: "Total Modules",     value: modules.length, icon: Layers,        color: "text-blue-600",   bg: "bg-blue-50" },
            { label: "Students Enrolled", value: modules.reduce((s, m) => s + m.course.courseEnrollments.length, 0), icon: Users,         color: "text-emerald-600", bg: "bg-emerald-50" },
            { label: "Assessments",       value: modules.reduce((s, m) => s + m.course.assessments.length, 0),       icon: ClipboardList, color: "text-violet-600", bg: "bg-violet-50" },
        ...
        <div className="mb-6 relative">
          ...
          <input type="text" placeholder="Search by name, code or program…" value={search}
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-dashed border-slate-300 bg-white py-20 text-center">
            ...
            <p className="font-medium text-slate-600">{search ? "No modules match your search" : "No modules yet"}</p>
            {!search && <p className="mt-1 text-sm text-slate-400">Add a module linked to an institution program.</p>}
          </div>
        ) : (
          <div className="space-y-8">
            {Object.entries(grouped).map(([programTitle, mods]) => {
              ...
              return (
                <div key={programTitle}>
                  ...
                  <div className="mb-3 flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-800">{programTitle}</span>
                    {field && (
                      <span className={`rounded-full border px-2.5 py-0.5 text-xs font-medium ${fieldColor(field)}`}>
                        {field.replace(/_/g, " ")}
                      </span>
                    )}
                      ...
                      >
                        ...
                        <div className="flex items-start gap-3 p-5 pb-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="font-semibold leading-snug text-slate-900">{mod.course.title}</p>
                            <div className="mt-1 flex flex-wrap items-center gap-1.5">
                              ...
                              {mod.course.orderIndex > 0 && (
                                <span className="text-xs text-slate-400">#{mod.course.orderIndex}</span>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mx-5 mb-4 flex items-center gap-3 rounded-lg bg-slate-50 px-3 py-2.5">
                          <span className="flex items-center gap-1.5 text-xs">
                            ...
                            <span className="font-bold text-slate-800">{mod.course.courseEnrollments.length}</span>
                            <span className="text-slate-400">students</span>
                          </span>
                          <span className="h-3 w-px bg-slate-200" />
                          <span className="flex items-center gap-1.5 text-xs">
                            <ClipboardList className="h-3.5 w-3.5 text-violet-500" />
                            <span className="font-bold text-slate-800">{mod.course.assessments.length}</span>
                        ...
                        <div className="flex items-center gap-1.5 border-t border-slate-100 p-3">
                          <button
                            onClick={() => router.push(`/dashboard/lecturer/modules/${mod.course.id}`)}
                          ...
                          <button
                            onClick={() => { if (confirm("Delete this module?")) deleteModule.mutate({ id: mod.course.id }); }}
                          ...
                          >
                            {deleteModule.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />}
                          </button>
                        </div>
                      </div>
                    ))}
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-slate-900">{editingId ? "Edit Module" : "Add New Module"}</h2>
                  <p className="text-xs text-slate-500">A course unit within an institution program</p>
                </div>
              </div>
              <button onClick={closeModal} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
            </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {approvedPrograms.map((p) => (
                        <option key={p.id} value={p.id}>{p.title} ({p.type.replace(/_/g, " ")})</option>
                      ))}
                    </select>
                    <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                  </div>
                  {approvedPrograms.length === 0 && (
                    <p className="mt-1 text-xs text-amber-600">No approved programs found for your institution.</p>
                  )}
                </Field>
              )}
              <Field label="Module Title" required>
                <input type="text" value={form.title} placeholder="e.g. Introduction to Databases"
                  onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} className={inputCls}
                />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="Module Code">
                  <input type="text" value={form.code} placeholder="e.g. CS301"
                    onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} className={inputCls}
                  />
                </Field>
                <Field label="Order Index">
                  <input type="number" value={form.orderIndex} min="0" placeholder="e.g. 1"
                    onChange={(e) => setForm((f) => ({ ...f, orderIndex: e.target.value }))} className={inputCls}
                  />
                </Field>
              </div>
              <Field label="Description">
                <textarea value={form.description} rows={3} placeholder="Brief description of this module…"
                  onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                  className={`${inputCls} resize-none`}
                />
              </Field>
              <button type="button" onClick={() => setForm((f) => ({ ...f, isMandatory: !f.isMandatory }))}
                className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition ${
                  form.isMandatory ? "border-blue-200 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600"
                }`}
              >
                <div className={`h-4 w-4 rounded-full border-2 transition ${form.isMandatory ? "border-blue-600 bg-blue-600" : "border-slate-300"}`} />
                Mandatory Module
              </button>
