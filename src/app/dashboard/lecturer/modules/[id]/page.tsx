...
export default function ModuleDetailPage() {
  ...
  const [linkType, setLinkType] = useState<"VIDEO_LINK" | "EXTERNAL_LINK">(
    "EXTERNAL_LINK",
  );
  ...
  const { data: course, isLoading: courseLoading } =
    api.course.getById.useQuery({ id: courseId }, { enabled: !!courseId });

  // Get sections based on view mode
  const { data: lecturerSections = [], isLoading: sectionsLoading } =
    api.courseResource.getSections.useQuery(
      { courseId },
      { enabled: !!courseId && viewMode === "lecturer" },
    );

  const { data: studentSections = [], isLoading: studentSectionsLoading } =
    api.courseResource.getStudentViewSections.useQuery(
      { courseId },
      { enabled: !!courseId && viewMode === "student" },
    );

  // Get modules for role info
  const { data: modules = [], isLoading: moduleLoading } =
    api.course.getMyModules.useQuery(undefined, { staleTime: 0 });
  const moduleData = modules.find((m) => m.course.id === courseId);

  // Get resources, assessments, students for lecturer view
  const { data: resources = [], isLoading: resourcesLoading } =
    api.courseResource.getByCourse.useQuery(
      { courseId: courseId ?? "" },
      { enabled: !!courseId && viewMode === "lecturer" },
    );

  const { data: assessments = [], isLoading: assessmentsLoading } =
    api.assessment.getByCourse.useQuery(
      { courseId: courseId ?? "" },
      { enabled: !!courseId && viewMode === "lecturer" },
    );
  ...
  const createAssessment = api.assessment.create.useMutation({
    onSuccess: () => {
      void utils.assessment.getByCourse.invalidate();
  ...
  const handleAssessmentSubmit = (e: React.FormEvent) => {
    ...
    if (!assessmentForm.title.trim() || !assessmentForm.type) {
      setAssessmentError("Title and type are required.");
      return;
    }
    const payload = {
      ...
      totalMarks: assessmentForm.totalMarks
        ? parseInt(assessmentForm.totalMarks)
        : null,
      passMarks: assessmentForm.passMarks
        ? parseInt(assessmentForm.passMarks)
        : null,
      weightPercent: assessmentForm.weightPercent
        ? parseFloat(assessmentForm.weightPercent)
        : null,
      dueDate: assessmentForm.dueDate
        ? new Date(assessmentForm.dueDate).toISOString()
        : null,
  ...
  const isLoading =
    courseLoading ||
    (viewMode === "lecturer" ? sectionsLoading : studentSectionsLoading);
  ...
  // After the queries, add this check
  if (courseLoading || sectionsLoading || studentSectionsLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <Loader2 className="h-8 w-8 animate-spin text-blue-500" />
      </div>
  ...
  const sections = viewMode === "lecturer" ? lecturerSections : studentSections;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    >
                      {course.isMandatory ? "Mandatory" : "Optional"} · Order #
                      {course.orderIndex}
                    </span>
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                        moduleData?.role === "LECTURER"
                          ? "bg-blue-100 text-blue-700"
                          : moduleData?.role === "CO_LECTURER"
          ...
          {/* Tabs - only show in lecturer view */}
          {viewMode === "lecturer" && (
            <div className="flex gap-1">
              {TABS.map((tab) => (
                ...
                >
                  ...
                  {tabBadges[tab.id] > 0 && (
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-bold ${activeTab === tab.id ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-600"}`}
                    >
                      {tabBadges[tab.id]}
                    </span>
                  )}
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        {viewMode === "student" ? (
          <StudentView
            ...
            onExitPreview={() => setViewMode("lecturer")}
          />
        ) : (
          // Lecturer View Content
          <>
            {/* ════ OVERVIEW ════ */}
            {activeTab === "overview" && (
                      ...
                      <div className="flex gap-3 pt-2">
                        ...
                        <button
                          onClick={() =>
                            updateModule.mutate({
                              id: courseId!,
                              title: overviewForm.title || undefined,
                              code: overviewForm.code || null,
                ...
                {assessmentsLoading ? (
                  ...
                  <div className="rounded-xl border border-dashed border-slate-300 bg-white py-20 text-center">
                    ...
                    <p className="mt-1 text-sm text-slate-400">
                      Create assignments, quizzes, and exams for this module.
                    </p>
                  </div>
                ) : (
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                  className={`rounded-full px-2.5 py-1 text-xs font-medium ${{ ASSIGNMENT: "bg-blue-100 text-blue-700", QUIZ: "bg-emerald-100 text-emerald-700", EXAM: "bg-red-100 text-red-700", PROJECT: "bg-violet-100 text-violet-700", PRESENTATION: "bg-orange-100 text-orange-700" }[a.type] ?? "bg-slate-100 text-slate-600"}`}
                                >
                                  {a.type}
                                </span>
                                {isOverdue && (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  if (confirm("Delete?"))
                                    deleteAssessment.mutate({ id: a.id });
                                }}
                            ...
                            <div className="mt-3 space-y-2">
                              ...
                              {a.submissions
                                ...
                                .map((sub) => (
                                  <div
                                    key={sub.id}
                                    className="flex items-center justify-between rounded-lg border border-amber-100 bg-amber-50 px-4 py-2.5"
                                  >
                                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium text-slate-900">
                              {e.student?.profile?.fullName ?? "Unknown"}
                            </p>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-bold text-slate-900">
                  {assessmentModal === "edit"
                    ? "Edit Assessment"
                    : "New Assessment"}
                </h2>
              </div>
              <button
                onClick={() => setAssessmentModal("closed")}
                className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            <form onSubmit={handleAssessmentSubmit} className="space-y-4 p-6">
              ...
              <Field label="Type" required>
                <select
                  ...
                  onChange={(e) =>
                    setAssessmentForm((f) => ({ ...f, type: e.target.value }))
                  }
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <Field label="Pass Marks">
                  <input
                    type="number"
                    min="0"
                    value={assessmentForm.passMarks}
                    placeholder="50"
                    onChange={(e) =>
                      setAssessmentForm((f) => ({
                        ...f,
      ...
      {gradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            <div className="space-y-4 p-6">
              <Field
                label={`Marks${gradeModal.totalMarks ? ` (out of ${gradeModal.totalMarks})` : ""}`}
                required
              >
                <input
                  type="number"
                  min="0"
                  max={gradeModal.totalMarks ?? undefined}
                  value={gradeMarks}
                  placeholder="0"
