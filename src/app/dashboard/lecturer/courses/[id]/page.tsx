...
import {
  GraduationCap, Loader2, ArrowLeft, FileText, Video,
  Link2, ExternalLink, Upload, Users, ClipboardList,
  DollarSign, Globe, Pencil, X, Plus, Trash2, Eye,
  EyeOff, AlertCircle, GripVertical, BookOpen, CheckCircle,
  Clock, BarChart3, Mail,
} from "lucide-react";
...
export default function CourseDetailPage() {
  ...
  const [overviewForm, setOverviewForm] = useState({ title: "", description: "", localPrice: "", foreignPrice: "" });
  ...
  const { data: courses = [], isLoading: courseLoading } = api.course.getMyCourses.useQuery();
  ...
  const updateCourse = api.course.update.useMutation({
    onSuccess: () => { void utils.course.getMyCourses.invalidate(); setEditingOverview(false); },
  });
  ...
  const createResource = api.courseResource.create.useMutation({
    onSuccess: () => {
      void utils.courseResource.getByCourse.invalidate();
      setLinkModal(false); setLinkUrl(""); setLinkTitle(""); setLinkDesc("");
    },
  });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await createResource.mutateAsync({
        courseId, title: file.name.replace(/\.[^.]+$/, ""),
        type: detectResourceType(file), fileUrl: result.url,
  ...
  const handleAssessmentSubmit = (e: React.FormEvent) => {
    ...
    if (editingAssessmentId) {
      updateAssessment.mutate({ id: editingAssessmentId, ...payload });
    } else {
      createAssessment.mutate({ courseId: courseId!, ...payload });
    }
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <button onClick={() => router.push("/dashboard/lecturer/courses")} className="mt-3 text-sm text-blue-600 hover:underline">Back to Courses</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="py-6">
            <button onClick={() => router.push("/dashboard/lecturer/courses")}
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    {course.localPrice && (
                      <span className="flex items-center gap-1 rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700">
                        <DollarSign className="h-3 w-3" />LKR {Number(course.localPrice).toLocaleString()}
                      </span>
                    )}
                  </div>
                </div>
              ...
              {editingOverview ? (
                <div className="space-y-4">
                  <Field label="Title" required>
                    <input type="text" value={overviewForm.title} className={inputCls}
                      onChange={(e) => setOverviewForm((f) => ({ ...f, title: e.target.value }))}
                    />
                  </Field>
                  <Field label="Description">
                    <textarea rows={5} value={overviewForm.description} className={`${inputCls} resize-none`}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Local Price (LKR)">
                      <div className="relative">
                        ...
                        <input type="number" min="0" step="0.01" value={overviewForm.localPrice}
                          onChange={(e) => setOverviewForm((f) => ({ ...f, localPrice: e.target.value }))}
                    ...
                    <Field label="Foreign Price (USD)">
                      <div className="relative">
                        ...
                        <input type="number" min="0" step="0.01" value={overviewForm.foreignPrice}
                          onChange={(e) => setOverviewForm((f) => ({ ...f, foreignPrice: e.target.value }))}
                          className={`${inputCls} pl-9`}
                        />
                      </div>
                    </Field>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    >Cancel</button>
                    <button onClick={() => updateCourse.mutate({ id: courseId!, title: overviewForm.title || undefined, description: overviewForm.description || null, localPrice: overviewForm.localPrice ? parseFloat(overviewForm.localPrice) : null, foreignPrice: overviewForm.foreignPrice ? parseFloat(overviewForm.foreignPrice) : null })}
                ...
                <div>
                  <div className="mb-4 flex items-start justify-between gap-4">
                    ...
                    <button
                      onClick={() => { setOverviewForm({ title: course.title, description: course.description ?? "", localPrice: course.localPrice?.toString() ?? "", foreignPrice: course.foreignPrice?.toString() ?? "" }); setEditingOverview(true); }}
                      className="flex flex-shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700"
                    >
                  ...
                  <div className="mt-6 flex flex-wrap gap-3 border-t border-slate-50 pt-5">
                    {course.localPrice ? (
                      <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-center">
                        <p className="text-xs text-slate-400">Local Price</p>
                        <p className="mt-0.5 text-base font-bold text-slate-900">LKR {Number(course.localPrice).toLocaleString()}</p>
                      </div>
                    ) : (
                      <div className="rounded-xl border border-green-100 bg-green-50 px-4 py-3 text-center">
                        <p className="text-xs text-green-600">Enrollment</p>
                        <p className="mt-0.5 text-base font-bold text-green-700">Free</p>
                      </div>
                    )}
                    {course.foreignPrice && (
                      <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-center">
                        <p className="text-xs text-slate-400">Foreign Price</p>
                        <p className="mt-0.5 text-base font-bold text-slate-900">USD {Number(course.foreignPrice).toLocaleString()}</p>
                      </div>
                    )}
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-center">
                      <p className="text-xs text-slate-400">Students</p>
                      <p className="mt-0.5 text-base font-bold text-slate-900">{students.length}</p>
                    </div>
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-center">
                      <p className="text-xs text-slate-400">Assessments</p>
                      <p className="mt-0.5 text-base font-bold text-slate-900">{assessments.length}</p>
                    </div>
                  </div>
                </div>
              )}
        ...
        {activeTab === "resources" && (
          <div>
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="font-bold text-slate-900">Course Resources</h2>
                <p className="text-sm text-slate-500">{resources.length} resource{resources.length !== 1 ? "s" : ""} · visible to enrolled students</p>
              </div>
              <div className="flex gap-2">
                <input ref={fileInputRef} type="file"
                  accept=".pdf,.ppt,.pptx,.jpg,.jpeg,.png,.gif,.webp,.mp4,.mov,.avi,.mkv"
            ...
            {resourcesLoading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-white py-20 text-center">
                ...
                <p className="mt-1 text-sm text-slate-400">Upload PDFs, videos, presentations, or add external links.</p>
              ...
              <div className="space-y-2">
                {resources.map((r, index) => (
                  <div key={r.id} className={`flex items-center gap-4 rounded-xl border bg-white px-5 py-4 shadow-sm transition ${
                    !r.isPublished ? "opacity-60 border-slate-100" : "border-slate-200"
                  }`}>
                    ...
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-slate-50 shadow-sm">
                    ...
                    <div className="flex flex-shrink-0 items-center gap-1">
                      ...
                      >
                        {r.isPublished ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                      </button>
                      <button
        ...
        {activeTab === "assessments" && (
          <div>
            <div className="mb-6 flex items-center justify-between">
              ...
              <button
                onClick={() => { setAssessmentForm({ title: "", type: "", totalMarks: "", passMarks: "", weightPercent: "", dueDate: "", instructions: "" }); setAssessmentError(null); setEditingAssessmentId(null); setAssessmentModal("create"); }}
            ...
            {assessmentsLoading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-white py-20 text-center">
                ...
                <p className="mt-1 text-sm text-slate-400">Create assignments, quizzes, and exams for this course.</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                              { ASSIGNMENT: "bg-blue-100 text-blue-700", QUIZ: "bg-emerald-100 text-emerald-700", EXAM: "bg-red-100 text-red-700", PROJECT: "bg-violet-100 text-violet-700", PRESENTATION: "bg-orange-100 text-orange-700" }[a.type] ?? "bg-slate-100 text-slate-600"
                            }`}>{a.type}</span>
                          ...
                          <div className="mt-1 flex flex-wrap items-center gap-x-4 text-xs text-slate-400">
                            ...
                            {a.weightPercent && <span>{Number(a.weightPercent)}% weight</span>}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => { if (confirm("Delete this assessment?")) deleteAssessment.mutate({ id: a.id }); }}
                            disabled={deleteAssessment.isPending}
                            className="rounded-lg bg-red-50 p-1.5 text-red-600 transition hover:bg-red-100 disabled:opacity-50"
                          >
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          {a.submissions.filter((s) => s.status === "SUBMITTED").map((sub) => (
                            <div key={sub.id} className="flex items-center justify-between rounded-lg border border-amber-100 bg-amber-50 px-4 py-2.5">
                              ...
                              <button
                                onClick={() => { setGradeModal({ submissionId: sub.id, studentName: sub.student?.profile?.fullName ?? "Student", totalMarks: a.totalMarks ?? null }); setGradeMarks(""); setGradeFeedback(""); setGradeStatus("GRADED"); }}
          ...
          <div>
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500">
                {students.filter((s) => s.status === "ACTIVE").length} active · {students.filter((s) => s.status === "COMPLETED").length} completed
              </p>
            </div>
            ...
            {studentsLoading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-white py-20 text-center">
                ...
                <p className="mt-1 text-sm text-slate-400">Students will appear here once they enroll in this course.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {students.map((e) => (
                  <div key={e.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
                    <div className="flex items-center gap-4">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700">
                        {e.student?.profile?.fullName?.charAt(0).toUpperCase() ?? "?"}
                      </div>
                      <div>
                        <p className="font-medium text-slate-900">{e.student?.profile?.fullName ?? "Unknown"}</p>
                        <div className="flex items-center gap-1 text-xs text-slate-400">
                          <Mail className="h-3 w-3" />{e.student?.profile?.email}
                        </div>
                        <p className="mt-0.5 text-xs text-slate-400">Enrolled {new Date(e.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      {e.grade && (
                        <div className="rounded-lg bg-violet-50 px-3 py-1.5 text-center">
                          <p className="text-xs text-violet-400">Grade</p>
                          <p className="text-sm font-bold text-violet-700">{e.grade}</p>
                        </div>
                      )}
                      <div>
                        <select
                          value={e.status}
                          onChange={(ev) => updateGrade.mutate({ courseEnrollmentId: e.id, status: ev.target.value as any })}
                          className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium outline-none"
                        >
            ...
            <div className="space-y-4 p-6">
              <div className="flex gap-2">
                {(["VIDEO_LINK", "EXTERNAL_LINK"] as const).map((t) => (
                  ...
                  >
                    {t === "VIDEO_LINK" ? "🎬 Video Link" : "🔗 External Link"}
                  </button>
                ))}
              </div>
              <Field label="Title" required>
                <input type="text" value={linkTitle} placeholder="e.g. Week 1 Lecture" onChange={(e) => setLinkTitle(e.target.value)} className={inputCls} />
              </Field>
              <Field label="URL" required>
                <input type="url" value={linkUrl} placeholder={linkType === "VIDEO_LINK" ? "https://youtube.com/..." : "https://..."} onChange={(e) => setLinkUrl(e.target.value)} className={inputCls} />
              </Field>
              <Field label="Description">
                <textarea rows={2} value={linkDesc} placeholder="Optional…" onChange={(e) => setLinkDesc(e.target.value)} className={`${inputCls} resize-none`} />
              </Field>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            ...
            <form onSubmit={handleAssessmentSubmit} className="space-y-4 p-6">
              <Field label="Title" required>
                <input type="text" value={assessmentForm.title} placeholder="e.g. Midterm Exam" onChange={(e) => setAssessmentForm((f) => ({ ...f, title: e.target.value }))} className={inputCls} />
              </Field>
              <Field label="Type" required>
                <div className="relative">
                  <select value={assessmentForm.type} onChange={(e) => setAssessmentForm((f) => ({ ...f, type: e.target.value }))} className={`${inputCls} appearance-none pr-8`}>
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  onClick={() => gradeSubmission.mutate({ submissionId: gradeModal.submissionId, marksObtained: parseFloat(gradeMarks), feedback: gradeFeedback || null, status: gradeStatus })}
